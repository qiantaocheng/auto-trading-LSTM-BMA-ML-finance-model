...
from .enhanced_temporal_validation import EnhancedPurgedTimeSeriesSplit, EnhancedValidationConfig, FeatureLagOptimizer, FeatureLagConfig
...
from .production_readiness_system import ProductionReadinessSystem, ProductionGates, ValidationMetrics
...
class BMAEnhancedConfig:
    ...
    validation_config: EnhancedValidationConfig = field(default_factory=EnhancedValidationConfig)
    
    # Regime detection settings
    regime_config: LeakFreeRegimeConfig = field(default_factory=LeakFreeRegimeConfig)
    
    # Factor decay settings
    factor_decay_config: FactorDecayConfig = field(default_factory=FactorDecayConfig)
    
    # Production gates settings
    production_gates: ProductionGates = field(default_factory=ProductionGates)
    ...
    knowledge_config: KnowledgeRetentionConfig = field(default_factory=KnowledgeRetentionConfig)
...
class BMAEnhancedIntegratedSystem:
    ...
    def __init__(self, config: BMAEnhancedConfig = None):
        ...
        self.feature_lag_optimizer = FeatureLagOptimizer(self.config.lag_config, self.config.validation_config)  # Inject unified config
            ...
            if cv_scores:
                ...
                if len(cv_scores) > 0 and fold_details:
                    ...
                    try:
                        ...
                        final_fold_calibration = self._final_fold_lgb_calibration(
                            X_train_final,
                            y_train_final,
                            X_test_final,
                            y_test_final,
                    ...
                    final_fold_calibration = self._final_fold_lgb_calibration(
                        X, y, X, y, sample_weights
                    )
                    logger.info(f"Final fold LGB calibration: IC={final_fold_calibration.get('ic', 0):.4f}")
                else:
                    final_fold_calibration = {}
            else:
    ...
    def _evaluate_cv_fold(self, X_train: pd.DataFrame, y_train: pd.Series,
                         X_test: pd.DataFrame, y_test: pd.Series,
                         sample_weights: pd.Series = None) -> Dict[str, float]:
        """Evaluate a single CV fold with lightweight LightGBM for consistency (Fix)"""
        from scipy.stats import pearsonr, spearmanr
        
        try:
            # Try LightGBM for consistent evaluation with final training
            try:
                import lightgbm as lgb
                
                # Lightweight LGB parameters for fast CV evaluation
                lgb_params = {
                    'objective': 'regression',
                    'boosting_type': 'gbdt',
                    'metric': 'rmse',
                ...
                # Create datasets
                train_set = lgb.Dataset(X_train.fillna(0), label=y_train, weight=sample_weights)
                
                # Quick training for CV (limited rounds)
                model = lgb.train(
                    lgb_params,
                    train_set,
                    num_boost_round=50,  # Limited for speed
                    callbacks=[lgb.log_evaluation(0)]
                )
                
                y_pred = model.predict(X_test.fillna(0))
                
                logger.debug("Using LightGBM for CV fold evaluation")
                
            except ImportError:
                # Fallback to Ridge if LightGBM not available
                from sklearn.linear_model import Ridge
                logger.debug("LightGBM not available, using Ridge fallback for CV")
    ...
    def _execute_model_training(self, prepared_data: Dict[str, Any],
                              ...
                              training_type: TrainingType) -> Dict[str, Any]:
        """Execute actual model training based on type"""
        
        logger.info(f"Executing {training_type.value} model training...")
        ...
        model_results = {'status': 'success', 'models': {}}
        
        # Train LightGBM if incremental trainer available
        if self.incremental_trainer:
        ...
        if self.incremental_trainer:
            # 🔥 FIX: Use real OOS manager instead of mock data
            from real_oos_manager import RealOOSManager
            
            # Initialize or get OOS manager
            if not hasattr(self, 'oos_manager'):
                self.oos_manager = RealOOSManager()
            ...
            if 'lightgbm' in model_results['models']:
                ...
                if lgb_model and hasattr(lgb_model, 'predict'):
                    ...
                    # Add to OOS manager
                    fold_id = f"fold_{training_type}_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}"
            ...
            if real_oos is not None and len(real_oos) > 50:
                ...
                # Fallback: Use validation predictions if insufficient OOS history
                logger.warning("Insufficient OOS history, using current validation for BMA update")
                fallback_oos = pd.DataFrame({
                    'target': y_valid,
                    'lightgbm_pred': lgb_pred if 'lgb_pred' in locals() else y_valid
                })
                bma_result = self.incremental_trainer.update_bma_weights_incremental(
