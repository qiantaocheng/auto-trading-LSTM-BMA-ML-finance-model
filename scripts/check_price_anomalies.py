...
def check_price_anomalies():
    ...
    if len(high_prices) > 0:
        ...
        for ticker in high_price_tickers[:10]:
            ...
            print(f"    {ticker}: 最高=${ticker_prices.max():.2f}, 均值=${ticker_prices.mean():.2f}")
    ...
    print(f"\n[4] 检查异常价格变动对target的影响:")
    print("-" * 80)
    
    # 计算10天收益率
    target_calc = df.groupby(level='ticker')['Close'].pct_change(10).shift(-10)
    ...
    if len(extreme_targets) > 0:
        ...
        for idx, val in top_targets.items():
            ...
            if len(ticker_data) > 0:
                ...
                # 找到t+10的价格
                date_plus_10 = pd.Timestamp(date) + pd.Timedelta(days=15)  # 大约10个交易日
    ...
    if len(extreme_targets) > 1000:
        print(f"\n[WARN] 发现大量异常target值（>50%）: {len(extreme_targets):,}个")
        print(f"  这些异常值可能导致Top N收益异常高")
        print(f"  建议:")
