#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""计算所有模型的 Overlap 胜率（包括 Top 5-15）"""
...
print("="*80)
print("所有模型的 Overlap 胜率计算（基于每日重叠观测）")
print("="*80)
...
print("\n【检查可用列】")
print("-"*80)
cb_cols = [c for c in cb_bucket.columns if 'top_' in c and 'return' in c and 'cum' not in c]
print(f"CatBoost 可用列: {cb_cols[:10]}...")
lr_cols = [c for c in lr_bucket.columns if 'top_' in c and 'return' in c and 'cum' not in c]
print(f"LambdaRank 可用列: {lr_cols[:10]}...")
...
# Calculate overlap win rates for different buckets
buckets_to_check = [
...
print("\n【CatBoost Overlap 胜率】")
print("="*80)
print(f"{'分桶':<15} {'总交易日数':<12} {'盈利日数':<12} {'亏损日数':<12} {'胜率':<10}")
print("-"*65)

cb_results = {}
for bucket_name, col_name in buckets_to_check:
    if col_name in cb_bucket.columns:
        ...
        print(f"{bucket_name:<15} {total:<12} {wins:<12} {losses:<12} {wr:<10.2f}%")
        ...
        print(f"{bucket_name:<15} {'N/A':<12} {'N/A':<12} {'N/A':<12} {'N/A':<10}")
...
for bucket_name, _ in buckets_to_check:
    ...
    print(f"{bucket_name:<15} {cb_str:<15} {lr_str:<15} {qqq_val:<15}")
