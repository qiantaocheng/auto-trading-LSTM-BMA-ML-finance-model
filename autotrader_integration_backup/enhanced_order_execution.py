...
class EnhancedOrderExecutor:
    ...
    async def _simulate_execution(self, order_id: str, symbol: str, side: str, 
        ...
        # Use limit price or get current market price
        if limit_price:
            execution_price = limit_price
        else:
            # Get actual market price for the symbol
            try:
                from .unified_trading_core import create_unified_trading_core
                core = create_unified_trading_core()
                quote = core.get_best_quote(symbol)
                
                if quote:
                    # Use bid/ask based on side
                    base_price = quote.ask if side == "BUY" else quote.bid
                else:
                    # Try to get price from polygon factors as fallback
                    try:
                        from .unified_polygon_factors import get_unified_polygon_factors
                        polygon_factors = get_unified_polygon_factors()
                        market_data = polygon_factors.get_current_price(symbol)
                        base_price = market_data.get('price', 100.0) if market_data else 100.0
                    except:
                        # Last resort fallback - log warning
                        logger.warning(f"No market data available for {symbol}, using fallback price")
                        base_price = 100.0  # Generic fallback, should not happen in production
        ...
        market_impact = abs(execution_price - base_price) / max(base_price, 1.0)  # Compare to actual base price
