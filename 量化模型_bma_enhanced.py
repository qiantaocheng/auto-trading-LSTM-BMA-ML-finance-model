...
# GPU加速优化（如果可用）
# 在导入前抑制 CuPy 对 CUDA_PATH 的提示并设置环境变量，避免无GPU环境噪声
os.environ.setdefault("CUPY_DISABLE_CUDA_WARNING", "1")
...
try:
    ...
    try:
        # 仅当存在可见 CUDA 设备时才启用
        from cupy.cuda import runtime as _cupy_rt  # type: ignore
        GPU_ACCELERATION = _cupy_rt.getDeviceCount() > 0
    except Exception:
        GPU_ACCELERATION = False
    ...
    print("[BMA GPU] GPU加速可用 (CuPy/cuDF)")
            ...
            # CuPy 可用但 cuDF 不可用时，仍然允许仅使用 CuPy
            print("[BMA GPU] 检测到CuPy，无cuDF，将在可用处使用GPU")
    else:
        print("[BMA GPU] 未检测到可用CUDA设备，使用CPU计算")
except Exception:
    GPU_ACCELERATION = False
    print("[BMA GPU] 无法导入CuPy，使用CPU计算")

# 尝试导入RAPIDS加速的sklearn（仅在GPU可用时尝试）
if GPU_ACCELERATION:
    try:
        from cuml import Ridge as cuRidge  # type: ignore
        from cuml import RandomForestRegressor as cuRandomForest  # type: ignore
