...
def _connect_with_retry(ib, host, port, client_id, timeout=15):
    """Connect to IBKR with retry on client ID collision."""
    import time
    last_err = None
    for attempt in range(MAX_CONNECT_RETRIES):
        ...
        try:
            ...
            last_err = e
            err_str = str(e).lower()
            if "already in use" in err_str or "client id" in err_str:
                print(f"Client ID {cid} in use, retrying with {cid + 1}...",
                      file=sys.stderr, flush=True)
                time.sleep(1)
                continue
            raise  # Non-client-ID error — don't retry
    raise last_err  # All retries exhausted


def cmd_buy(args):
    ib = IB()
    _connect_with_retry(ib, args.host, args.port, args.client_id)
    try:
        ...
        if limit_price and limit_price > 0:
            ...
            fill_timeout = getattr(args, 'fill_timeout', 30) or 30
        else:
        ...
        for _ in range(fill_timeout * 2):
            ib.sleep(0.5)
            if trade.isDone():
                break

        # Cancel remaining order if partial fill to prevent ghost positions
        remaining = int(trade.orderStatus.remaining or 0)
        if remaining > 0 and not trade.isDone():
            try:
                ib.cancelOrder(trade.order)
                ib.sleep(2)
                print(f"Cancelled remaining {remaining} shares for {args.symbol.upper()}", file=sys.stderr)
            except Exception as ce:
                print(f"Cancel failed: {ce}", file=sys.stderr)

        # Re-read status AFTER cancel to avoid stale "Submitted" → phantom positions
        ...
        return {"success": False, "error": str(e), "symbol": args.symbol.upper(), "action": "BUY",
                ...
                "filled_qty": 0, "remaining_qty": 0, "total_qty": args.quantity}
    finally:
        ib.disconnect()
...
def cmd_sell(args):
    ...
    try:
        ...
        limit_price = getattr(args, 'limit_price', None)
        if limit_price and limit_price > 0:
            order = LimitOrder("SELL", args.quantity, limit_price)
            fill_timeout = getattr(args, 'fill_timeout', 30) or 30
        else:
        ...
        # Cancel remaining order if partial fill to prevent qty drift
        remaining = int(trade.orderStatus.remaining or 0)
        if remaining > 0 and not trade.isDone():
            try:
                ib.cancelOrder(trade.order)
                ib.sleep(2)
                print(f"Cancelled remaining sell {remaining} shares for {args.symbol.upper()}", file=sys.stderr)
            except Exception as ce:
                print(f"Cancel failed: {ce}", file=sys.stderr)
    ...
    try:
        ...
        for av in ib.accountValues():
            ...
            if av.tag == "NetLiquidation":
                ...
                buying_power = _safe_float(av.value)
            elif av.tag == "GrossPositionValue":
                gross_pos_value = _safe_float(av.value)
        margin_used_pct = (gross_pos_value / net_liq) if net_liq > 0 else 0.0
        return {
            "cash": cash,
            "net_liq": net_liq,
        ...
        return {"cash": 0.0, "net_liq": 0.0, "buying_power": 0.0,
                "margin_used_pct": 0.0, "error": str(e)}
    finally:
        ib.disconnect()
...
def main():
    try:
        ...
        buy_parser.add_argument("--limit-price", type=float, default=0.0,
                               help="Limit price (0 = market order)")
        buy_parser.add_argument("--fill-timeout", type=int, default=30,
                               help="Fill timeout in seconds")
        ...
        sell_parser.add_argument("--limit-price", type=float, default=0.0,
                               help="Limit price (0 = market order)")
        sell_parser.add_argument("--fill-timeout", type=int, default=30,
                               help="Fill timeout in seconds")
        ...
        # Ensure C# always gets valid JSON, even on unexpected errors
        print(json.dumps({"success": False, "error": f"Unhandled: {e}",
                          "order_id": 0, "symbol": "", "action": "",
