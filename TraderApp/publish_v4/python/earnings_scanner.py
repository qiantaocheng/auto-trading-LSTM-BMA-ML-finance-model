...
def main():
    ...
    parser.add_argument("--lookback-days", type=int, default=2,
                        help="How many days back to fetch news (default=2 to cover T-1 only)")
    ...
    # T+0 to T-2: today, yesterday, and 2 days ago
    today = datetime.now(tz=None).date()
    since_date = (today - timedelta(days=lookback)).strftime("%Y-%m-%d")
    valid_dates = set()
    for d in range(lookback + 1):  # 0, 1, 2
        valid_dates.add((today - timedelta(days=d)).strftime("%Y-%m-%d"))

    emit_progress("Date range", 8, f"Scanning news from {since_date} to {today.strftime('%Y-%m-%d')}")
