...
# -*- coding: utf-8 -*-
"""
AutoTrader专用的Polygon统一因子入口
集成所有Polygon因子功能，提供统一接口
"""
...
try:
    ...
    logging.warning(f"导入Polygon模块失败: {e}")

logger = logging.getLogger(__name__)

class PolygonUnifiedFactors:
    """
    AutoTrader专用的Polygon统一因子管理器
    集成所有因子计算、风控和交易决策功能
    """
    ...
    def _initialize_components(self):
        ...
        try:
            # 初始化Polygon客户端
            if 'polygon_client' in globals():
                self.polygon_client = polygon_client
                logger.info("Polygon客户端已连接")
            
            # 初始化因子组件
            if 'PolygonFactorIntegrator' in globals():
                self.factor_integrator = PolygonFactorIntegrator()
                logger.info("Polygon因子集成器已初始化")
            
            if 'PolygonShortTermFactors' in globals():
                self.short_term_factors = PolygonShortTermFactors()
                logger.info("Polygon短期因子已初始化")
            
            if 'PolygonCompleteFactors' in globals():
                self.complete_factors = PolygonCompleteFactors()
                logger.info("Polygon完整因子库已初始化")
            
            # 初始化风控收益平衡器
            if 'get_risk_balancer_adapter' in globals():
                self.risk_balancer = get_risk_balancer_adapter()
                logger.info("风控收益平衡器已初始化")
            
            self.enabled = True
            logger.info("Polygon统一因子管理器初始化完成")
    ...
    def calculate_short_term_factors(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"计算{symbol}短期因子失败: {e}")
            self.stats['failed_calculations'] += 1
            return {}
    ...
    def calculate_complete_factors(self, symbol: str, categories: List[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"计算{symbol}完整因子失败: {e}")
            self.stats['failed_calculations'] += 1
            return {}
    ...
    def check_trading_conditions(self, symbol: str) -> Dict[str, Any]:
        """
        检查交易条件(替代原有的超短期判断)
        使用Polygon数据进行流动性、价差、动量等检查
        """
        if not self.is_enabled():
            return {'can_trade': False, 'reason': 'Polygon未启用'}
        
        try:
            # 获取基础信息
            basic_info = self.get_stock_basic_info(symbol)
            if not basic_info:
                return {'can_trade': False, 'reason': '无法获取基础信息'}
            
            # 获取历史数据
            hist_data = self.get_historical_data(symbol, days=30)
            if hist_data is None or len(hist_data) < 20:
                return {'can_trade': False, 'reason': '历史数据不足'}
            
            # 价格检查
            current_price = float(hist_data['Close'].iloc[-1])
            if current_price < 5.0:
                return {'can_trade': False, 'reason': f'价格过低: ${current_price:.2f}'}
            ...
            if avg_dollar_volume < 500000:  # 50万美元
                return {'can_trade': False, 'reason': f'成交量不足: ${avg_dollar_volume:.0f}'}
            ...
            logger.error(f"检查{symbol}交易条件失败: {e}")
            return {'can_trade': False, 'reason': f'检查失败: {e}'}
    ...
    def process_signals_with_risk_control(self, signals: Union[pd.DataFrame, List[Dict]]) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"风控处理{len(df_signals)}个信号，生成{len(orders)}个订单")
            return orders
            
        except Exception as e:
            logger.error(f"风控信号处理失败: {e}")
            return self._process_signals_basic(signals)
