...
def create_freshness_scoring(
    config: Optional[FreshnessConfig] = None,
    tau_minutes: Optional[float] = None,
    max_age_minutes: Optional[float] = None,
    base_threshold: Optional[float] = None,
    freshness_threshold_add: Optional[float] = None
) -> DataFreshnessScoring:
    """创建数据新鲜度评分实例"""
    if config is None:
        # 如果没有提供config，使用参数创建新的配置
        config = FreshnessConfig()
        
        # 更新提供的参数
        if tau_minutes is not None:
            config.tau_minutes = tau_minutes
        if max_age_minutes is not None:
            config.max_age_minutes = max_age_minutes
        if base_threshold is not None:
            config.base_threshold = base_threshold
        if freshness_threshold_add is not None:
            config.freshness_threshold_add = freshness_threshold_add
