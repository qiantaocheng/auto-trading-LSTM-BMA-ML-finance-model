...
"""
线程安全事件循环管理器
解决Tkinter GUIandasyncio事件循环线程安全问题
"""
...
class ThreadSafeEventLoopManager:
    ...
    def start(self, timeout: float = 10.0) -> bool:
        """start事件循环（线程安全）"""
        if self._running:
            self.logger.warning("事件循环in运行")
            return True
        
        self.logger.info("Starting event loop manager...")
        ...
        if not self._started.wait(timeout):
            self.logger.error("事件循环start超when")
            self._cleanup()
            return False
        
        if self._error:
            self.logger.error(f"事件循环startfailed: {self._error}")
            return False
        
        self._stats['start_time'] = time.time()
        self.logger.info("Event loop manager started successfully")
        return True
    ...
    def _run_event_loop(self):
        ...
        try:
            ...
            self.logger.info(f"Event loop started in thread {threading.current_thread().name}  started")
    ...
    def _execute_command(self, cmd: Dict[str, Any]):
        ...
        try:
            ...
            self.logger.error(f"执行命令failed: {e}")
            self._stats['errors'] += 1
            self._response_queue.put({
                'id': cmd_id,
                'status': 'error',
        ...
        while True:
            try:
                ...
                try:
                    ...
                    if response['id'] == cmd_id:
                        if response['status'] == 'success':
                            ...
                            raise asyncio.CancelledError("协程be取消")
                        else:
                ...
                if timeout and (time.time() - start_time) > timeout:
                    ...
                    raise TimeoutError(f"协程执行超when: {timeout} seconds")
                
                time.sleep(0.01)  # 避免CPU过度使use
    ...
    def get_task_result(self, task_id: str, timeout: Optional[float] = None) -> Any:
        ...
        while True:
            try:
                ...
                if response['id'] == task_id:
                    if response['status'] == 'success':
                        ...
                        raise asyncio.CancelledError("任务be取消")
                    else:
                        raise response.get('error', Exception("未知错误"))
            except queue.Empty:
                pass
            ...
            if timeout and (time.time() - start_time) > timeout:
                raise TimeoutError(f"retrieval结果超when: {timeout} seconds")
