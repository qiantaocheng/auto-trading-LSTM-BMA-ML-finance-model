...
class UnifiedOrderValidator:
    ...
    def __init__(self, config_manager=None):
        ...
        self.validation_config = {
            "min_order_value": 0.0,         # 移除最小订单金额限制
            "max_order_value": 1000000.0,   # 提高至100万美元
            "min_price": 0.01,              # 最低价格0.01美元
            "max_price": 10000.0,           # 最高价格1万美元
            "max_position_pct": 0.95,       # 单仓位最高95%
            "per_trade_risk_pct": 0.50,     # 单笔交易风险50%
            "max_daily_orders": 1000        # 每日最多1000笔订单
        }
    ...
    async def _perform_validation(self, symbol, side, quantity, price, account_value) -> ValidationResult:
        ...
        try:
            ...
            # 移除最小订单金额检查，允许任何金额的订单
            
            if order_value > self.validation_config["max_order_value"]:
                return ValidationResult(False, f"订单金额过大: ${order_value:.2f}")
            
            # 3. 价格范围验证 - 放宽限制
            if price < self.validation_config["min_price"]:
                return ValidationResult(False, f"价格过低: ${price:.4f}")
            if price > self.validation_config["max_price"]:
                return ValidationResult(False, f"价格过高: ${price:.2f}")
            
            # 4. 账户余额验证 - 允许负余额账户交易（保证金账户）
            # 移除账户余额检查，允许保证金交易
            
            # 5. 风险比例验证 - 大幅放宽
            if account_value > 0:  # 只在有正余额时检查比例
                risk_ratio = order_value / account_value
                if risk_ratio > self.validation_config["max_position_pct"]:
                    self.logger.warning(f"高风险交易: {symbol} 风险比例 {risk_ratio:.2%}")
                    # 不拒绝，仅记录警告
            else:
                # 负余额或零余额时跳过比例检查
                risk_ratio = 0.0
