...
class UnifiedQuantCore:
            ...
            with self.safe_lock("load_market_data"):
                ...
                if missing_tickers:
                    ...
                    if data_source == "polygon":
                        ...
                        new_data = self._load_fallback_data(missing_tickers, start_date, end_date)
    ...
    def _load_polygon_data(self, tickers: List[str], start_date: str, end_date: str) -> Dict[str, pd.DataFrame]:
        ...
        try:
            ...
            self.logger.warning("Polygon客户端不可用，使用备用数据源")
            return self._load_fallback_data(tickers, start_date, end_date)
    
    def _load_fallback_data(self, tickers: List[str], start_date: str, end_date: str) -> Dict[str, pd.DataFrame]:
        """备用数据源"""
        # 生成模拟数据（扩展时间范围以支持技术指标计算）
        data = {}
        start_dt = pd.to_datetime(start_date)
        extended_start = start_dt - pd.Timedelta(days=150)  # 向前扩展150天
        dates = pd.date_range(extended_start, end_date, freq='D')
        
        for ticker in tickers:
            np.random.seed(hash(ticker) % 2**32)  # 确保一致性
