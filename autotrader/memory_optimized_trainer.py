    ...
    def combine_batch_results(self, batch_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for batch_result in batch_results:
            ...
            if 'feature_importance' in batch_result:
                for feature, importance in batch_result['feature_importance'].items():
                    if feature not in combined_result['feature_importance']:
                        combined_result['feature_importance'][feature] = []
                    combined_result['feature_importance'][feature].append(importance)
            
            # 收集批次摘要
            if 'batch_metadata' in batch_result:
                metadata = batch_result['batch_metadata']
                combined_result['batch_summary'].append(metadata)
                combined_result['total_training_time'] += metadata.get('training_time_seconds', 0)
                combined_result['total_stocks'] += metadata.get('tickers_count', 0)
        
        # 计算平均性能指标
        for model_name, performances in combined_result['model_performance'].items():
            if performances:
                combined_result['model_performance'][model_name] = {
                    'mean_score': np.mean(performances),
                    'std_score': np.std(performances),
                    'scores': performances
                }
        
        # 计算平均特征重要性
        for feature, importances in combined_result['feature_importance'].items():
            if importances:
                combined_result['feature_importance'][feature] = {
