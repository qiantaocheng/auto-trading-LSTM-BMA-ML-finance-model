...
class OrderStateMachine:
    ...
    def __init__(self, order_id: int, symbol: str, side: str, quantity: int, 
        ...
        if self.state_change_callback:
            try:
                ...
                self.logger.warning(f"状态回调failed: {e}")
    ...
    def transition(self, new_state: OrderState, metadata: Optional[Dict[str, Any]] = None, 
        ...
        with self._lock:
            if not self._is_valid_transition(self.state, new_state):
                self.logger.warning(f"no效状态转换: {self.state.value} -> {new_state.value}")
                return False
            ...
            if self.state_change_callback:
                try:
                    ...
                    self.logger.warning(f"状态变化回调failed: {e}")
    ...
    def _update_fill_info(self, fill_data: Dict[str, Any]):
        ...
        self.logger.info(f"executionupdates: {self.filled_quantity}/{self.quantity} @ {self.avg_fill_price}")
    
    def update_fill(self, filled_qty: int, avg_price: float) -> bool:
        """updatesexecution信息并转换状态"""
        with self._lock:
            fill_data = {
            ...
            if filled_qty >= self.quantity:
                # 完全execution
                return self.transition(OrderState.FILLED, fill_data, "完全execution")
            elif filled_qty > self.filled_quantity:
                # 部分execution
                return self.transition(OrderState.PARTIAL, fill_data, "部分execution")
            
            return False
    
    def cancel(self, reason: str = "use户取消") -> bool:
        """取消订单"""
    ...
    def fail(self, reason: str = "执行failed") -> bool:
        """订单failed"""
...
class OrderManager:
    ...
    def _audit_state_change(self, order_sm: 'OrderStateMachine', old_state: Optional[OrderState], 
        ...
        try:
            ...
            self.logger.warning(f"审计回调failed: {e}")
    ...
    async def create_order(self, order_id: int, symbol: str, side: str, quantity: int,
        ...
        async with self._lock:
            if order_id in self.orders:
                raise ValueError(f"订单ID {order_id} 存in")
            ...
            order_sm = OrderStateMachine(
                ...
                strategy=strategy,
                parent_id=parent_id,
                state_change_callback=self._audit_state_change  # settings审计回调
            )
    ...
    async def update_order_state(self, order_id: int, new_state: OrderState, 
        ...
        if not order_sm:
            self.logger.warning(f"未找to订单: {order_id}")
            return False
