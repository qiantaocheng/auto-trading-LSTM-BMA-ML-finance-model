...
- `Simple17FactorEngine.compute_all_17_factors()` (`bma_models/simple_25_factor_engine.py:268`) computes the 17/T+10 alpha features, adds `Close`, and enforces a strict `MultiIndex(['date','ticker'])` before optional sentiment enrichment. This is the "get data, calculate features, into MultiIndex" stage
...
5. **LambdaRankStacker**: LambdaRank with ranking optimization
   - Input: Alpha factors (10-25 features)
...
1. **Data Loading**:
   - Loads MultiIndex parquet file: `polygon_factors_all_filtered.parquet`
...
3. Generates predictions using trained models:
   - First-layer models predict on alpha factors
...
**Important**: Prediction uses `self.meta_ranker_stacker.predict()` (`bma_models/量化模型_bma_ultra_enhanced.py:5717-5722`). No RidgeStacker is used in prediction.

### Prediction Data Format
- **Input**: MultiIndex DataFrame with `(date, ticker)` index
- **Required Columns**: Alpha factors (same as training features)
...
First Layer Training (PurgedCV)
    ...
    ├── CatBoost
    ├── LightGBM Ranker
    └── LambdaRankStacker (500 rounds, 70 early stopping)
    ↓
