...
class AlphaStrategiesEngine:
    ...
    def combine_alphas(self, alpha_df: pd.DataFrame, weights: pd.Series) -> pd.Series:
        ...
        """
        # 确保权重对齐
        aligned_weights = weights.reindex(alpha_df.columns, fill_value=0.0)
        aligned_weights = aligned_weights / aligned_weights.sum()  # 重新标准化
        
        # 加权平均
        combined_signal = (alpha_df * aligned_weights).sum(axis=1)
